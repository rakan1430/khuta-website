/* ============================================================
   ...
   (١) رقم مطابقة من ست خانات (كان أربعاً، فرفعه الفحص الأمني: أربع خانات
       = ١٠٬٠٠٠ احتمال يمكن تجريبها كلها داخل نافذة التسعين ثانية. ومعه
       قفلٌ بعد خمس محاولات خاطئة، ومولّد تعموي آمن بدل random()).
...
async function submitQrApproval(){
    ...
    if(!/^[0-9]{6}$/.test(code)){ setMsg("الرقم ست خانات.", "bad"); return; }

    try{
        /* ⚠️ المخالفة تعود قيمةً لا استثناءً — عمداً.
           كان الرفض يُرفع بـraise في قاعدة البيانات، وraise يُجهض المعاملة
           فيُلغى معها عدّاد المحاولات الذي زِيد قبله مباشرة. أي أن القفل بعد
           خمس محاولات لم يكن يقفل شيئاً، وكان تجريب الرمز كلّه ممكناً. */
        const { data, error } = await sb.rpc("screen_login_approve", {
            p_public_id: publicId, p_pair_code: code,
        });
        if(error) throw error;
        if(data === "CODE_MISMATCH"){
            setMsg("الرقم لا يطابق الشاشة. تأكّد أنك تنظر إلى السبورة التي تريد فتحها.", "bad");
            return;
        }
        if(data === "TOO_MANY_ATTEMPTS"){
            setMsg("محاولات خاطئة كثيرة — أُبطل هذا الرمز. أنشئ رمزاً جديداً من الشاشة.", "bad");
            return;
        }
        if(data !== "OK") throw new Error(String(data || "UNKNOWN"));
    }catch(e){
