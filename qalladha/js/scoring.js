...
(function (global) {
  ...
  const BAND = 5;          // مرونة ± خمسة أطر: تسامحٌ مع اختلاف التوقيت
  ...
  function partPercent(v) {
    return Math.round(clamp((v - 0.15) / 0.75, 0, 1) * 100);
  }
  ...
  function compare(fa, fb) {
    ...
    /* رفع الأسّ يشدّ التمييز: القيم المتوسّطة (تشابه عابر بين أي
       صوتين عريضي الطيف) تهبط، والتشابه الحقيقي يبقى عالياً. */
    const tone = Math.pow(clamp(alignSim(fa.mel, fb.mel), 0, 1), 1.6);
