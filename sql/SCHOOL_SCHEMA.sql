...
/* ============================================================
   دلاء التخزين وسياساتها
   ------------------------------------------------------------
   ⚠️ الدلاء لا تُهاجَر مع الجداول. عند دمج مشروعَي Supabase نُقلت الجداول
   ولم ينتقل معها دلو teacher-files، فكان رفع أي ملف يفشل رغم سلامة الكود.
   إن نقلت المشروع مستقبلاً فأنشئ الدلاء يدوياً وأعد سياساتها.

   teacher-files : خاص، ٢٥ ميجا، أي نوع.  مسار: <school>/<member>/<uuid>.<ext>
   exam-images   : خاص، ٥ ميجا، صور فقط.  نفس شكل المسار.

   ⚠️ سياسة قراءة teacher-files لا تكرّر قاعدة "من يرى الملف" بل تسأل:
   "هل يستطيع هذا المستخدم رؤية صفّ الملف في teacher_files؟" — والاستعلام
   الفرعي يخضع لسياسات ذلك الجدول، فتُطبَّق قاعدته حرفياً. المحاولة الأولى
   أعادت كتابة القاعدة فأخطأت (ظنّتها تطابق المرحلة، وهي في الحقيقة أن يكون
   صاحب الملف معلّماً لأحد فصول الطالب). نسختان من قاعدة واحدة تفترقان حتماً.
   ============================================================ */


/* ============================================================
   درسان في RLS دفعنا ثمنهما، لا تكرّرهما
   ------------------------------------------------------------
   (١) السياسات المتساهلة تُجمَع بـOR لا بـAND.
       إضافة سياسة أشدّ بجانب سياسة أوسع لا تُشدّد شيئاً: يكفي أن تمرّ
       الأوسع. كانت على teacher_quick_login سياسة tql_owner تسمح لأي عضو
       نشط بالكتابة، فأبطلت السياسة الجديدة تماماً وسمحت لطالب بتفعيل دخول
       سريع لنفسه. ➡️ عند التشديد: احذف القديمة، لا تضع الجديدة بجانبها.

   (٢) سياستان تسأل كلٌّ منهما جدول الأخرى = دوران لا نهائي.
       سياسة الكتابة على exam_assignments كانت تسأل teacher_exams، وسياسة
       قراءة الطالب على teacher_exams تسأل exam_assignments. النتيجة:
       "infinite recursion detected in policy" — أي أن **إرسال أي اختبار
       كان يفشل دائماً**، وهي الميزة الأساسية، ولم يظهر ذلك إلا باختبار
       جلسة محاكاة.
       ➡️ العلاج: دوال SECURITY DEFINER تجيب بنعم/لا وتتجاوز سياسات الجدول
       الآخر فتكسر الدورة: i_own_exam, i_am_in_exam_school,
       exam_is_assigned_to_me.

   ⚠️ ولهذا لا تكفي قراءة السياسة بالعين: هذان العطلان كانا يبدوان سليمين
   تماماً في الكود، ولم يظهرا إلا بتشغيل جلسات محاكاة تختبر ما يجب أن ينجح
   وما يجب أن يُرفض.
   ============================================================ */


/* ============================================================
   ما الذي يستطيعه الإداري بجلسة كلمة مرور؟ — قرار يحتاج مراجعتك
   ------------------------------------------------------------
   الوضع الحالي: الإداري بكلمة مرور ينشئ الفصول ويعدّل الجدول ويقرأ كل شيء،
   لكن **إضافة الأعضاء تحتاج تأكيد Google** (سياسة members_write_admin
   تشترط google_verified).

   السبب: إضافة الأعضاء هي مفتاح المنصة كلها — من يضيف عضواً يستطيع أن
   يضيف نفسه بأي دور. فجعلناها أعلى الأفعال حماية.

   الأثر عملياً: الإداري يدخل من رابطه بكلمة مروره ويعمل عادياً، وحين يضيف
   عضواً يظهر له شريط "جلسة محدودة" فيضغط "تأكيد بحساب Google" مرة، ثم
   تُقبل إضافاته اثنتي عشرة ساعة.

   ➡️ إن رأيت هذا متشدّداً وأردت أن تكفي كلمةُ المرور للإداري في كل شيء،
      فالتغيير سطر واحد (حذف and google_verified() من members_write_admin)
      لكن اعلم ثمنه: تسريب كلمة مرور إداري واحد = سيطرة كاملة على المدرسة.
   ============================================================ */
