...
with intentionally_public(proname) as (
    values
        ...
        ('school_for_request'),
        -- معرّف المدرسة يُقرأ بلا حساب: الموقع يحتاجه قبل الدخول ليعرف لأي
        -- مدرسة يُرسل الطلب. لا يكشف شيئاً حساساً — اسم المدرسة معروف أصلاً.
        -- ⚠️ كانت غائبة عن هذه القائمة رغم أنّها مُصرَّح بها عمداً في
        -- SCHOOL_SCHEMA.sql (grant execute … to anon) — فكان هذا الفحص
        -- سيُبلّغ عن صفٍّ سليم بوصفه ثغرة. أُضيفت هنا لتطابق الشفرة الفعلية.
        ('school_id_by_slug'),
